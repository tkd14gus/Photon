...
using Photon.Pun;       //포톤네크워크 핵심기능!!!
using Photon.Realtime;  //포톤 서비스 관련(룸옵션, 디스커넥션 등)

//네트워크 매니져 : 룸(게임공간)으로 연결시켜주는 역할
//포톤네트워크 : 마스터서버 -> 로비(대기실) -> 룸(게임공간)

//MonoBehaviourPunCallbacks : 포톤서버 접속, 로비접속, 룸접속 등 이벤트를 받아준다.
public class NetworkManager : MonoBehaviourPunCallbacks
{

    public Text infoText;   //네트워크 상태를 보야즐 텍스트
    ...
    public void OnConnect()
    {
        ...
        {
            //마스터 서버에 좁속중이 아니라면 다시 마스터서버에 접속 시도하기
            //접속 정보 표시하기
            infoText.text = "오프라인 : 마스터 서버와 연결 실패 \n 접속 재시도 중....";
